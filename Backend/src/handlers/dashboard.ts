...
export const getProfit = async (req, res) => {
    try {
        ...
        const customerCol = await database_connection(['vendor invoice', 'customer invoice'])

        const costPromise = customerCol[0].aggregate([
            {
                $match: {
                    userId: req.user.id,
            ...
            {$group: {_id: null, sum: {$sum: "$total"}}}]).toArray()
        const salePromise = customerCol[1].aggregate([
            {
                $match: {
                    userId: req.user.id,
                    date: {$gt: lowerDate, $lt: upperDate}
...
export const getStates = async (req, res) => {
    try {
        ...
        const collections = await database_connection(['customer invoice', 'vendor invoice'])
        const [sales, expense] = await Promise.all([
            collections[0].aggregate([
                {
                    $match: {
                        userId: req?.user?.id,
                        date: {$gt: lowerDate, $lt: upperDate}
