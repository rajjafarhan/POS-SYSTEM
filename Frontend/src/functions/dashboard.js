...
export const fetchDashboardData = async ({queryKey}) => {
  ...
  const bearer = "Bearer " + token;
    const res = await axios.get(`http://localhost:3000/api/dashboard/getstates/${year}`,{
        headers:{
...
export const fetchMonthStates = async ({queryKey}) => {
  ...
  const token = localStorage.getItem("token");
  const bearer = "Bearer " + token;
    const res = await axios.get(`http://localhost:3000/api/dashboard/getprofit/${month}/${year}`,{
        headers:{
            authorization:bearer
        }
    })
