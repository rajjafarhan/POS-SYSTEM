...
export const getAllVendors = async (req, res) => {
    try {
      ...
      const vendors = await vendorcollection[0]
        ...
        .toArray();
        const totalVendorCount = await vendorcollection[0]
      ...
      return res.status(200).json({ vendors:vendors, totalCount: totalVendorCount });
    } catch (error) {
...
export const createVendor = async (req, res) => {
  try {
    ...
    if (result.acknowledged === true) {
      ...
      return res.status(201).json({ message: 'Vendor data inserted successfully' });
    } else {
...
export const deleteVendor = async (req, res) => {
  ...
  export const deleteAllVendors = async (req, res) => {
    try {
      ...
      const deleteResult = await vendorcollection[0].deleteMany({ userId: req.user.id });
