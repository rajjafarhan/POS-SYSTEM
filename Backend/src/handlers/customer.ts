...
export const getAllCustomers = async (req, res) => {
    try {
      ...
      const customers = await custCollection[0]
        ...
        .toArray();
        const totalCustomerCount = await custCollection[0]
      ...
      return res.status(200).json({ customers:customers, totalCount: totalCustomerCount });
    } catch (error) {
...
export const deleteAllCustomers = async (req, res) => {
    try{
        ...
        if (result.deletedCount >= 1) {
            return res.status(200).json({ message: 'All customers deleted successfully' });
          } else {
